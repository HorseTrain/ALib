...
namespace DCpu.Translation
{
    public class Translator
    {
        ...
        public unsafe void Execute(State.ExecutionContext context, ulong address)
        {
        ...
        public unsafe ulong ExecuteRaw(ulong address, IntPtr context)
        {
            if (!Open)
            {
                if (Interlocked.Increment(ref _threadCount) == 1)
